locals {
  rabbitmq_endpoint = aws_mq_broker.main.instances[0].endpoints[0]
  rabbitmq_host     = split(":", trimprefix(local.rabbitmq_endpoint, "amqps://"))[0]
  database_url      = "jdbc:postgresql://${aws_db_instance.main.address}:5432/ticketing?sslmode=require"
  ...
  application_environment = [
    ...
    { name = "RABBITMQ_SSL_ENABLED", value = "true" },
    { name = "SEED_DEMO", value = "false" }
  ]

  application_secrets = [
    { name = "DATABASE_PASSWORD", valueFrom = "${aws_secretsmanager_secret.application.arn}:database_password::" },
    { name = "RABBITMQ_PASSWORD", valueFrom = "${aws_secretsmanager_secret.application.arn}:rabbitmq_password::" },
